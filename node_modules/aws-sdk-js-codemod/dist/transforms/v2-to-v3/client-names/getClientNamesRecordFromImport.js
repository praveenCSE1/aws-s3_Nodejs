...
const getClientNamesRecordFromImport = (j, source, clientNamesFromDeepImport) => {
    ...
    const specifiersFromNamedImport = (0, modules_1.getImportSpecifiers)(j, source, config_1.PACKAGE_NAME).filter((specifier) => specifier?.type === "ImportSpecifier");
    ...
    for (const clientName of clientNamesFromDeepImport) {
        ...
        const specifiersFromDeepImport = (0, modules_1.getImportSpecifiers)(j, source, deepImportPath).filter((specifier) => ["ImportDefaultSpecifier", "ImportNamespaceSpecifier"].includes(specifier?.type));
